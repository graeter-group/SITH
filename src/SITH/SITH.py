...
class SITH:
    """A class to calculate & house SITH analysis data. Publicly accessable
    variables & methods:
    ...
    deltaQ : numpy.ndarray
        RIC value matrix  of deformed - reference [DOF index, deformation
        index]
    ...
    setKillAtoms(atoms: list[int]):
        Marks atoms to be ignored and removed from the reference geometry upon
        extraction.
    setKillDOFs(dofs: list[Tuple]):
        Marks DOFs to be ignored and removed from the reference geometry upon
        extraction.
    removeMismatchedDOFs():
        Removes DOFs from the deformed geometry which do not appear in the
        reference geometry upon extraction.
    extractData():
        Indicates that any killing of atoms and DOFs is finished, begins
        extraction and organization of data from input .fchk files.
    energyAnalysis():
    ...
    def __init__(self, rePath='', dePath=''):
        """
        Takes in the reference geometry .fchk file path and the deformed
        geometry .fchk file path or path to directory of deformed geometries
        .fchk files.
        ...
        self.energies = None
        """Stress energy associated with a DOF for each deformation
        ...
        """
        Total stress energy associated with going from the reference structure
        to the ith deformed structure.
        ...
        self.pEnergies = None
        """
        Proportion of stress energy in each degree of freedom to the each
        structure's total stress energy in the form [DOF index, deformation
        index]
        ...
        instead implement the method setReference() and the associated
        ...
        Notes
        -----
        Publicly accessable for retrieval and reference, please refrain from
        manually setting this value as other analysis variables depend upon it.
        """
        ...
        qF[d.o.f. index, row or deformed geometry index] -> value of d.o.f. for
        ...
        self.deltaQ = None
        """
        Matrix of changes in RIC values from reference geometry to each
        deformed geometry.

        deltaQ[d.o.f. index, row or deformed geometry index] -> value of d.o.f.
        for deformed geometry at index of self.deformed
        ...
        self._kill = False
        """ Sentinel value indicating if any atoms of DOFs present in the input
        files should be removed from geometries, hessian, and analysis."""
...
# region Atomic Homicide
    ...
    def __kill(self):
        ...
        """Use Case Note:
        This is a private method to limit user error. Specification of these
        DOFs is made by the user programmatically with the public functions
        SetKillAtoms(atoms: list) and SetKillDOFs(dofs: list) prior to data
        extraction by calling SITH.extractData(). If no mismatch between number
        of DOFs in each geometry's coordinates and Hessian, this can be
        manually called after extractData() as well but is not recommended."""
...
# region Public Functions meant to be used by novice-intermediate user
    ...
    def hessian(self):
        """
        Returns Hessian matrix used to calculate the stress energy analysis,
        ...
        retrieval purposes, please refrain from manually setting it. If you
        would like to manually change this value for a SITH object, instead
        implement the method setReference() and the associated refactoring
        recommended in its documentation.
        """
        return self.reference.hessian

    def setKillAtoms(self, atoms: list):
        """
        Takes in the atoms which should be removed from the reference geometry,
        must be used prior to calling SITH.extractData(), which also runs
        removeMismatchedDOFs.
        """
    ...
    def setKillDOFs(self, dofs: list):
        """
        Takes in the DOFs (degrees of freedom) which should be removed from the
        reference geometry, must be used prior to calling SITH.extractData(),
    ...
    def removeMismatchedDOFs(self):
        """
        Removes any DOFs which are not in the reference geometry to ensure that
        all geometries have the correct DOFs.

        -----
        NOTE: If there are DOFs in the reference geometry that do not exist in
        the deformed geometry, it will mess up this method and indicates an
        issue with the calculation which must be fixed in the generation of
        input. Solution: https://gaussian.com/gic/ specify Active GIC in
        deformed opt
        """
        print("Removing DOFs in the deformed geometries which are not " +
              "present in the reference geometry...")
        for deformation in self._deformed:
            ...
            for j in range(max(deformation.dims[0], self._reference.dims[0])):
                if j < deformation.dims[0]:
                    ...
                    if deformation.dimIndices[j] not in \
                       list(self._reference.dimIndices):
                        dofsToRemove.append(j)
                if j < self._reference.dims[0]:  # reference not in deformed
                    assert self._reference.dimIndices[j] in \
                           deformation.dimIndices, \
                           f"Deformed geometry ({deformation.name}) is " +\
    ...
    def extractData(self, ignore_last=None):
        ...
        previously specified by the user with SetKillAtoms or SetKillDOFs
        This method must always be called prior to energyAnalysis to extract
        and set up the relevant data.
        """
    ...
    def energyAnalysis(self):
        ...
        for i in range(len(self._deformed)):
            ...
            self.deformationEnergy[0, i] = 0.5 *\
                                           np.transpose(self.deltaQ[:, i]).\
                                           dot(self._reference.hessian).\
                                           dot(self.deltaQ[:, i])
            for j in range(self._reference.dims[0]):
                isolatedDOF = np.hstack((np.zeros(j), self.deltaQ[j, i],
                                         np.zeros(self._reference.dims[0]-j-1)
                                         ))
    ...
    def setReference(self, geometryName: str):
        ...
        geometry to the deforation list. If the specified geometry does not
        exist or has no Hessian, the reference geometry will simply not be
        changed. Deformation energy is still calculated for all geometries
        aside from the new reference geometry.
        """

        """ Implementation Instructions:
        To implement this without re-extracting the data (which is basically
        the current workflow of creating a new SITH instead), you must set the
        SITH._reference geometry to that with the new geometryName, move the
        old reference geometry into the list of deformed geometries, and then
        _validateGeometries() _populateQ(). This seems unnecessary as the cost
        which it saves is minimal.
        """
        raise NotImplementedError(
            "Unimplemented due to current lack of necessity, " +
            "contact @mmfarrugia on github for more info.")
        """ Use Case Note:

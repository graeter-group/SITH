...
def writeAll(sith: SITH, filePrefix="") -> bool:
    return writeSummary(sith, filePrefix) and \
           writeDeltaQ(sith, filePrefix) and \
           writeEnergyMatrix(sith, filePrefix) and \
           writeError(sith, filePrefix)


def writeSummary(sith: SITH, filePrefix='', includeXYZ=False) -> bool:
    """
    Takes in SITH object sith, Writes summary.txt file of sith data. Returns
    True if successful
    -----
...
def writeTotEnergiesString(sith: SITH, filePrefix="") -> True:
    """
    Takes in SITH object sith, Writes the change in RICs per structure.
    Returns true if successful.
    -----
    Data is in Angstroms and Radians and of the format row:DOF
    column:deformation structure.
    """
    try:
...
def writeDeltaQ(sith: SITH, filePrefix="") -> bool:
    """
    Takes in SITH object sith, Writes the change in RICs per structure, Returns
    true if successful.
    -----
    Data is in Angstroms and Radians and of the format row:DOF
    column:deformation structure.
...
def writeEnergyMatrix(sith: SITH, filePrefix='') -> bool:
    """
    Takes in SITH object sith, Writes the energy in each degree of freedom per
    deformed geometry, Returns True if successful
    -----
    ...
    try:
        ...
        with open(sith._referencePath.parent.as_posix() +
                  sith._referencePath.root + filePrefix + 'E_RICs.txt',
                  "w") as dq:
            dq.writelines('\n'.join(ePrint))
            dq.write('\n')
        return True
...
def writeXYZ(geometry: Geometry):
    ...
    write(str(geometry._path.parent.as_posix() + geometry._path.root +
              geometry._path.stem+".xyz"),
          geometry.atoms, format='xyz', append=False)
...
def buildTotEnergiesString(sith: SITH) -> list:
    """
    Takes in SITH object sith, Returns a list of strings containing error
    information per deformed geometry. Data is in Hartrees and percentages.
    """
    assert sith.deformationEnergy is not None, \
           "SITH.energyAnalysis() has not been performed yet, no " +\
...
def buildDeltaQString(sith: SITH) -> list:
    """
    Takes in SITH object sith, Returns a list of strings containing the change
    in internal coordinates in each degree of freedom per deformed geometry.
    Data is in Angstroms and degrees of the format row:DOF column: deformation
    """
    """
...
def buildInternalCoordsString(sith: SITH) -> list:
    ...
    indices involved in each degree of freedom.
    """
...
def buildEnergyMatrix(sith: SITH) -> list:
    """
    Takes in SITH object sith, Returns a list of strings containing the energy
    in each degree of freedom per deformed geometry. Data is in Hartrees and
    of the format row:DOF column:deformation
    """
    """
    DOF Index       Deformation 1       Deformation 2       ...
    1               stress E            stress E            ...
...
def compareEnergies(sith: SITH) -> Tuple:
    ...
    assert sith.deformationEnergy is not None, \
           "SITH.energyAnalysis() has notbeen performed yet, no summary " +\
           "information available."
